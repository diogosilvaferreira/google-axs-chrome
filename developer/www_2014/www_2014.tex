...
the STEM subject, in particular in late secondary and tertiary stage. As
...
challenging problem as expressions can be arbitrarily complex in the sense that
there is no limit to how deep sub-expressions are nested or how many layers of
parenthesis are used. Moreover, a large number of unusual symbols are used that
can have different meaning in different mathematical areas. An expression like
the following taken from~\cite{knuthart} {\small
\begin{equation*}
...
automatically is considerably more difficult. And already well known examples
like the quadratic formula can pose be non-trivial to translate to speech.
\begin{equation}
  \label{eq:quadratic}
...
which either are not web-ready solutions or offer limited customizability that
has to be done by users rather than by the providers of the web
content. Moreover, the main challenge, how to enable a user to engage with
mathematical content that goes beyond the speech translation of entire
expressions has never been fully addressed.

One of the main contributions of our approach is to offer users a way to explore
interactively mathematical expressions (Sec.~\ref{sec:explore}) on the web
accessible, where it is given in a variety of ways and formats
(Sec.~\ref{sec:math}). We do this by exploiting and extending some of
ChromeVox's unique features (detailed in Sec.~\ref{sec:chromevox}) that allow
users to engage with rich web applications. It has led to an implementation of a
flexible speech rule engine that enables us to customize the reading experience
along several axes and that also provides an API for easy adaptation to
specialized content by users and web site authors (Sec.~\ref{sec:translate}).
It also makes it possible employ a more effective representation in lieu of a
web element which we shall exploit for semantically more correct interpretation
of formulas (Sec.~\ref{sec:alternative}).
...
or spacing, as well as specialized attributes such as for fonts, accents, etc.
