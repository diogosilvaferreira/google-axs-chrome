/**
 * @fileoverview Caret browsing script.
 *
 ...
 * @type {Node}
 */
CaretBrowsing.editableNode = document.body;
...
CaretBrowsing.isControlThatNeedsArrowKeys = function(node) {
  ...
  if (node == CaretBrowsing.editableNode || node != document.activeElement) {
    return false;
  }
...
CaretBrowsing.setInitialCursor = function() {
  ...
  var start = new Cursor(CaretBrowsing.editableNode, 0, '');
  var end = new Cursor(CaretBrowsing.editableNode, 0, '');
  var nodesCrossed = [];
  var result = cvox.TraverseUtil.getNextChar(start, end, nodesCrossed, true);
  if (result == null) {
    return;
  }
...
CaretBrowsing.setFocusToNode = function(node) {
  while (node && node != CaretBrowsing.editableNode) {
...
CaretBrowsing.flashCaretElement = function() {
  ...
  CaretBrowsing.editableNode.appendChild(vert);

  var horiz = document.createElement('div');
  horiz.className = 'CaretBrowsing_FlashHoriz';
  horiz.style.left = (x - 20) + 'px';
  horiz.style.top = (y + height / 2 - 6) + 'px';
  horiz.style.width = (40) + 'px';
  horiz.style.height = '11px';
  CaretBrowsing.editableNode.appendChild(horiz);

  window.setTimeout(function() {
    CaretBrowsing.editableNode.removeChild(horiz);
    CaretBrowsing.editableNode.removeChild(vert);
    if (CaretBrowsing.caretElement) {
      CaretBrowsing.setCaretElementNormalStyle();
    }
  }, 250);
...
CaretBrowsing.createCaretElement = function() {
  console.log('createCaretElement');
...
CaretBrowsing.updateCaretOrSelection = function(scrollToSelection) {
  if (!CaretBrowsing.isCaretVisible)
    return;
 ...
 */
CaretBrowsing.moveByWord = function(evt) {
...
CaretBrowsing.forwards = function(cursor, nodesCrossed) {
  ...
  var result = cvox.TraverseUtil.forwardsChar(cursor, nodesCrossed);
  ...
  if (!cvox.DomUtil.isDescendantOfNode(cursor.node, CaretBrowsing.editableNode)) {
    cursor.node = previousCursor.node;
    cursor.index = previousCursor.index;
    cursor.text = previousCursor.text
    result = null;
  }
...
CaretBrowsing.backwards = function(cursor, nodesCrossed) {
  var previousCursor = cursor.clone();
  var result = cvox.TraverseUtil.backwardsChar(cursor, nodesCrossed);
...
CaretBrowsing.moveRight = function(evt) {
  ...
  while (true) {
    var result;
    if (CaretBrowsing.moveByWord(evt)) {
      result = cvox.TraverseUtil.getNextWord(previousEnd, end, nodesCrossed);
    } else {
...
CaretBrowsing.onExtensionMessage = function(request) {
  ...
  CaretBrowsing.onEnable = request['onenable'];
  CaretBrowsing.onJump = request['onjump'];
 ...
 * message listeners.
 */
CaretBrowsing.init = function(node) {
  console.log('init');
  CaretBrowsing.isWindowFocused = document.hasFocus();

  // Tell TraverseUtil not to traverse the caret element.
  cvox.TraverseUtil.SKIP_CLASS = 'CaretBrowsing_Caret';

  if (node) {
    CaretBrowsing.editableNode = node;
    node.addEventListener('keydown', CaretBrowsing.onKeyDown, false);
    node.addEventListener('click', CaretBrowsing.onClick, false);
  } else {
