...
\conferenceinfo{W4A}{2014 --- Technical, April 7-9, 2014, Seoul, Korea.\\
Co-Located with the 23nd International World Wide Web Conference.}
\CopyrightYear{2014}
\crdata{978-1-4503-2651-3}
...
\BeginAccSupp{method=plain,Alt={Faa di Bruno's formula. This is an identity that
    generalises the chain rule from calculus to higher order derivatives.  The
    right hand side is a nested double sum with multi-indices over a large
    fraction with complex n-ary multiplicative and factorial expressions.  In
    addition over and underbraces are added for explanation of the single
    component.}}
 {\small
\begin{equation*}
...
\BeginAccSupp{method=plain,Alt={The quadratic formula x equals minus b plus
    minus square root over b square minus four a c divided by two
    a.}} 
\begin{equation}
  \label{eq:quadratic}
...
\BeginAccSupp{method=plain,Alt={The MathML epresentation of the quadratic
    formula.}}
\begin{figure}[t!]
  \begin{center}
    \leavevmode
...
\end{lstlisting}
    \caption{MathML for the quadratic formula~(\ref{eq:quadratic}).}
...
Fig.~\ref{fig:quadratic-mml} presents our example of the quadratic
...
MathML. However, currently only a few browsers and ePub readers support MathML
and rendering is either achieved by specialist browser plugins or by third party
libraries.
...
%    \captionof{figure}{MathJax representations of~(\ref{eq:quadratic}) from
    \caption{MathJax representations of the quadratic formula~(\ref{eq:quadratic}) from
...
\end{lstlisting}
\caption{Image markup for the quadratic formula~(\ref{eq:quadratic}) from 
  \protect\url{en.wikipedia.org/wiki/Quadratic_equation}.}
...
%     \begin{center}
%       \leavevmode
%       \BeginAccSupp{method=plain,Alt={One large triangle containing four smaller
%           blue triangles}}
%       \includegraphics[width=2cm]{images/granularity1}
%       \EndAccSupp{}
%       \qquad
%       \BeginAccSupp{method=plain,Alt={One large triangle containing five smaller
%           red triangles, where three overlap the previously blue triangles.}}
...
%     \leavevmode
%     \BeginAccSupp{method=plain,Alt={One large triangle containing five smaller
%         blue triangles with arrows between triangles from left to right.}}
%     \includegraphics[width=2cm]{images/walker1}
%     \EndAccSupp{}
%     \qquad
%     \BeginAccSupp{method=plain,Alt={One large triangle containing the same
%         smaller triangles triangles as before, where one is no longer blue and
%         arrows go between the other four in a different order.}}
...
% \begin{center}
%     \leavevmode
%     \BeginAccSupp{method=plain,Alt={One large triangle containing a smaller blue
%         triangle. There are additional smaller triangles in different colours
%         behind the large triangle and dashed arrows between them and the blue
%         triangle, symbolising that they can be exchanged.}}
...
superscript is equal to $2$: \tttrule{self::mathml:msup; ./*[2][text()=2]}{[n]
  ./*[1]; [t] "square"} The rule's action specifies that first the content of the
...
this by way of example. Consider the following speech rule for \texttt{mfenced}
elements:

\noindent\texttt{self::mathml:mfenced;\\
string-length(string(@separators))=1 ${\rightarrow}$
[t] @open;\\
\hspace*{\fill}[m] ./* (separator:@separators); [t] @close}

\noindent The rule is applicable to any \texttt{mfenced} node that has a
\texttt{separators} attribute that contains exactly one character. Its action
has the notable features that
\begin{inparaenum}[(a)]
\item the first and last utterance are constructed by Xpath expressions
  ...
  respectively. And
\item that the multi component has a separator annotation, which specifies a
...
Suppose we want to read the following logical inference rule double negation
correctly, that is given in MathML on the right:
...
\BeginAccSupp{method=plain,Alt={A tree containing the MathML representation of
    the quadratic formula. The nodes are tags, and the leaves are tags with
    mathematical symbols.}}
\begin{tikzpicture}[scale=.68, transform shape,
  level 1/.style={sibling distance=4cm,level distance=.8cm},
  level 4/.style={sibling distance=4cm,level distance=1.2cm}
]
\node[draw] {math}
  %%[edge from parent fork down]
  [grow via three points={one child at (0,-1.25) and two children at (-1.3,-1.1) and (1.3,-1.1)}, edge from parent fork down]
    child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mi\\\textcolor{black}{x}\end{center}\end{minipage}}}
    child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mo\\\textcolor{black}{=}\end{center}\end{minipage}}}
    child {node[draw] {mfrac}[grow via three points={one child at (0,-1.5) and two children at (-1.5,-1) and (5.5,-1)}, edge from parent fork down]
      child {node[draw] {mrow}[grow via three points={one child at (0,-1.5) and two children at (-1,-1.35) and (1,-1.35)}, edge from parent fork down]
        child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mo\\\textcolor{black}{-}\end{center}\end{minipage}}}
        child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mi\\\textcolor{black}{b}\end{center}\end{minipage}}}
        child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mo\\\textcolor{black}{$\pm$}\end{center}\end{minipage}}}
        child {node[draw] {msqrt}[grow via three points={one child at (0,-1.5) and two children at (-1,-1.45) and (1,-1.45)}, edge from parent fork down]
          child {node[draw] {msup}[grow via three points={one child at (0,-1.5) and two children at (-1,-1.45) and (1,-1.45)}, edge from parent fork down]
            child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mi\\\textcolor{black}{b}\end{center}\end{minipage}}}
            child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mn\\\textcolor{black}{2}\end{center}\end{minipage}}}
          }
          child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mo\\\textcolor{black}{-}\end{center}\end{minipage}}}
          child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mn\\\textcolor{black}{4}\end{center}\end{minipage}}}
          child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mi\\\textcolor{black}{a}\end{center}\end{minipage}}}
          child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mi\\\textcolor{black}{c}\end{center}\end{minipage}}}
        }
      }
      child {node[draw] {mrow}[grow via three points={one child at (0,-1.5) and two children at (-1,-1) and (.5,-1)}, edge from parent fork down]
        child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mn\\\textcolor{black}{2}\end{center}\end{minipage}}}
        child {node[draw]{\begin{minipage}{3.25ex}\begin{center}mi\\\textcolor{black}{a}\end{center}\end{minipage}}}
      }
    }
...
\begin{tikzpicture}[scale=.68, transform shape,    
  ...
  \node[draw] {=}
  [grow via three points={one child at (0,-1.25) and two children at (-2.3,-1) and (2.3,-1)}, edge from parent fork down]
  child {node[draw] {x}}  
  child {node[draw] {/}[grow via three points={one child at (0,-1.3) and two children at (-1.3,-1) and (5.5,-1)}, edge from parent fork down]
    child {node[draw] {$\pm$}[grow via three points={one child at (0,-1.3) and two children at (-1,-1) and (3,-1)}, edge from parent fork down]
      child {node[draw] {-}[grow via three points={one child at (0,-1) and two children at (-1,-1) and (1,-1)}, edge from parent fork down]
        child {node[draw]{b}}}
      child {node[draw] {sqrt}[grow via three points={one child at (0,-1) and two children at (-1,-1) and (1,-1)}, edge from parent fork down]
        child {node[draw] {-}[grow via three points={one child at (0,-1) and two children at (-2,-1) and (2,-1)}, edge from parent fork down]
          child {node[draw] {$\,\widehat{}\,$}[grow via three points={one child at (0,-1) and two children at (-1,-1) and (1,-1)}, edge from parent fork down]
            child {node[draw]{b}}
            child {node[draw]{2}}}
          child {node[draw] {$\cdot$}[grow via three points={one child at (0,-1) and two children at (-1,-1) and (1,-1)}, edge from parent fork down]
            child {node[draw]{4}}
            child {node[draw]{a}}
            child {node[draw]{c}}
        }
...
% \bibliographystyle{plain} \bibliography{w4a}
\begin{thebibliography}{10}

\bibitem{DBLP:conf/hci/DoushP09}
...
\bibitem{Bates:2010:SMU:1880751.1880821}
...
\bibitem{google:chromevox-webstore}
...
\bibitem{conf/icchp/GillanBKP04}
...
\newblock {\em ICCHP'04}, LNCS 3118. Springer, 2004.
