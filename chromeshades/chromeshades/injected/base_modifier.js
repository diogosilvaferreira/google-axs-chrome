 ...
 * @param {Element} sheet The stylesheet element.
...
/**
 * Utility function to iterate over all nodes in the page and call an
 * action function on each one.
 * @param {Function} action The function to call on each node.
 */
cvox.BaseModifier.prototype.forAllNodes = function(action) {
  ...
  while (node && node != document.body) {
    ...
    while (!node.nextSibling && node != document.body) {
 ...
 */
cvox.BaseModifier.prototype.processDOM = function() {
  ...
  var indentStack = [];
  var indentRoot = {};
  indentRoot.node = document.body;
  indentRoot.indent = 0;

  // Loop over all nodes
  this.forAllNodes(goog.bind(function(node, computedStyle) {
...
cvox.BaseModifier.prototype.processNode = function(
  ...
  if (node.constructor == HTMLButtonElement ||
    ...
    if (node.hasAttribute('aria-label')) {
      ...
      if (target) {
        node.innerText = cvox.DomUtil.getText(target);      
      }
